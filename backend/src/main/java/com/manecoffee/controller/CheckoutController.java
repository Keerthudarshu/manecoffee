...
import com.manecoffee.dto.CartItemDTO;
import com.manecoffee.dto.OrderReviewDTO;
import com.manecoffee.entity.*;
import com.manecoffee.repository.AddressRepository;
import com.manecoffee.repository.CheckoutSelectionRepository;
import com.manecoffee.service.CartService;
...
public class CheckoutController {
    ...
    public ResponseEntity<?> placeOrder(@RequestParam("email") String email) {
        try {
            ...
            return ResponseEntity.ok(new com.manecoffee.dto.OrderDTO(order));
        } catch (IllegalArgumentException e) {
            logger.error("Invalid request for user: {}", email, e);
            return ResponseEntity.badRequest().body(e.getMessage());
        } catch (RuntimeException e) {
