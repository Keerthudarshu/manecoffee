    ...
    return (
        <div className={`pt-12 pb-6 bg-white border-t border-border/50 ${className}`}>
            <div className="container mx-auto px-4">
                {/* Certificates Row */}
                <div className="flex flex-row items-center justify-center gap-6 sm:gap-16 md:gap-24 mb-10">
                    {certificates.map((cert) => (
                        <div key={cert.name} className="flex-shrink-0 flex flex-col items-center">
                            <img
                                src={cert.src}
                                alt={`${cert.name} Certificate`}
                                className="h-14 sm:h-24 md:h-32 w-auto object-contain transition-all duration-300"
                                style={{
