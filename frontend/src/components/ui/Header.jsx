            ...
            <nav className="hidden lg:flex items-center space-x-8">
              {navigationItems.map((item, index) => (
                <div key={index} className="relative group">
                  <Link
                    to={item.path}
                    className="flex items-center gap-2 px-4 py-3 text-[#120d07] hover:text-[#C9A227] font-semibold transition-all duration-300 relative group"
                    onClick={(e) => {
                      if (item.hasDropdown) {
                        e.preventDefault();
                  ...
                  >
                    <Icon name={item.icon} size={18} className="text-[#120d07] group-hover:text-[#C9A227]" />
                    <span>{item.label}</span>
                    {item.hasDropdown && (
                      <Icon name="ChevronDown" size={16} className="transition-transform duration-200 group-hover:rotate-180 text-[#120d07] group-hover:text-[#C9A227]" />
                    )}
                    <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C9A227] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
                  </Link>
            ...
            <nav className="container mx-auto px-4 py-6">
              <div className="space-y-2">
                {navigationItems.map((item, index) => (
                  <Link
                    key={index}
                    to={item.path}
                    className="flex items-center gap-3 px-4 py-3 text-[#120d07] hover:text-[#C9A227] hover:bg-muted/30 rounded-xl transition-all duration-300"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    <Icon name={item.icon} size={20} className="text-[#120d07]" />
              ...
              <div className="mt-6 pt-6 border-t border-border">
                <Link
                  ...
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  <Icon name="User" size={18} className="text-[#120d07]" />
                  <span className="font-semibold">{loggedIn ? "My Account" : "Login"}</span>
                </Link>
              </div>
            </nav>
          </div>
